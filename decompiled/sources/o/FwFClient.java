package o;

import java.math.BigInteger;

/* JADX INFO: loaded from: classes4.dex */
public abstract class FwFClient {
    public static final BigInteger RemoteActionCompatParcelizer = BigInteger.valueOf(0);
    public static final BigInteger IconCompatParcelizer = BigInteger.valueOf(1);

    static {
        BigInteger.valueOf(2L);
        BigInteger.valueOf(3L);
        new BigInteger("", 16);
        BigInteger.valueOf(743L).bitLength();
    }
}
