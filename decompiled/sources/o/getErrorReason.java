...
public abstract class getErrorReason {
    public static final BrazeSdkAuthenticationErrorEvent read;
    ...
    static {
        read = (SimpleValueCallback.write && SimpleValueCallback.read && !onError.IconCompatParcelizer()) ? new BrazeSdkAuthenticationErrorEvent(1) : new BrazeSdkAuthenticationErrorEvent(0);
    }
    ...
    public static int RemoteActionCompatParcelizer(byte[] bArr, int i, int i2) {
        ...
        if (i3 == 1) {
            return RemoteActionCompatParcelizer(b, bArr[i]);
        }
        if (i3 == 2) {
