...
public class ActivityRecognitionMappingManager {
    ...
    private boolean isUpdate(ActivityRecognitionMappingInfo activityRecognitionMappingInfo) {
        for (int i = 0; i < this.activityRecognitionMappingInfoList.size(); i++) {
            if (this.activityRecognitionMappingInfoList.get(i).equals(activityRecognitionMappingInfo)) {
                ...
                WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "need update arMappingInfo:time:" + activityRecognitionMappingInfo2.getDetectionIntervalMillis() + "->" + activityRecognitionMappingInfo.getDetectionIntervalMillis());
    ...
    public long getMinTime() {
        ...
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "getMinTime ,minTime is " + j);
        return j;
    }
    ...
    private void callBackWithReport(ActivityRecognitionMappingInfo activityRecognitionMappingInfo, ActivityRecognitionResult activityRecognitionResult) {
        try {
            ...
            WrappedCompositionsetContent1211.serializer(TAG, "Failed to get activity recognition callback");
        }
    }

    public void addActivityUpdatesMappingInfo(long j, ARCallback aRCallback, ClientInfo clientInfo) {
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "addActivityUpdatesMappingInfo enter, detectionIntervalMillis:" + j + ", callback:" + aRCallback);
        ...
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "addActivityUpdatesMappingInfo success, infos size:" + this.activityRecognitionMappingInfoList.size());
    }
    ...
    public void removeActivityUpdatesMappingInfo(ARCallback aRCallback) {
        WrappedCompositionsetContent1211.IconCompatParcelizer(TAG, "removeActivityUpdatesMappingInfo enter, callback:" + aRCallback);
        if (aRCallback == null) {
            WrappedCompositionsetContent1211.read(TAG, "callback is null");
            return;
        }
        ...
        WrappedCompositionsetContent1211.IconCompatParcelizer(TAG, "removeActivityUpdatesMappingInfo success,  infos size:" + this.activityRecognitionMappingInfoList.size());
    }

    public List<ActivityRecognitionMappingInfo> removeActivityUpdatesMappingInfoByPackageName(String str) {
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "removeActivityUpdatesMappingInfoByPackageName:enter:" + str);
        ArrayList arrayList = new ArrayList();
        for (ActivityRecognitionMappingInfo activityRecognitionMappingInfo : this.activityRecognitionMappingInfoList) {
        ...
        this.activityRecognitionMappingInfoList.removeAll(arrayList);
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "removeActivityUpdatesMappingInfoByPackageName:exit:" + str);
        return arrayList;
    }

    public void send(List<DetectedActivity> list) {
        WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "send begin");
        if (list.size() > 0) {
            ...
            for (ActivityRecognitionMappingInfo activityRecognitionMappingInfo : this.activityRecognitionMappingInfoList) {
                WrappedCompositionsetContent1211.RemoteActionCompatParcelizer(TAG, "sending");
