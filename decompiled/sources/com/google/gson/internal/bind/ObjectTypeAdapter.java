    ...
    public Object read2(JsonReader jsonReader) throws IOException {
        ...
        while (true) {
            if (jsonReader.hasNext()) {
                ...
                Object terminal = objTryBeginNesting2 == null ? readTerminal(jsonReader, jsonTokenPeek2) : objTryBeginNesting2;
                if (objTryBeginNesting instanceof List) {
                    ((List) objTryBeginNesting).add(terminal);
                } else {
                    ((Map) objTryBeginNesting).put(strNextName, terminal);
                }
                if (z) {
                    arrayDeque.addLast(objTryBeginNesting);
                    objTryBeginNesting = terminal;
                }
            } else {
                if (objTryBeginNesting instanceof List) {
                    jsonReader.endArray();
