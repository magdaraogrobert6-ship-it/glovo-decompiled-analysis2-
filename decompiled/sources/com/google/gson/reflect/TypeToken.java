...
public class TypeToken<T> {
    ...
    public static TypeToken<?> getParameterized(Type type, Type... typeArr) {
        ...
        if (!(type instanceof Class)) {
            Gson$$ExternalSyntheticBUOutline0.m(type, "rawType must be of type Class, but was ");
            return null;
        }
        ...
        if (length2 != length) {
            FieldType$$ExternalSyntheticBUOutline0.serializer(length, length2, cls.getName(), " requires ", " type arguments, but got ");
            return null;
        }
        if (typeArr.length == 0) {
            return get(cls);
        ...
        if (GsonTypes.requiresOwnerType(type)) {
            r8lambdayUqGVEpC1kg8ua3jc9a113shNIE.read("Raw type ", cls.getName(), " is not supported because it requires specifying an owner type");
            return null;
        }
        for (int i = 0; i < length; i++) {
            ...
            for (Type type3 : typeVariable.getBounds()) {
                if (!GsonTypes.getRawType(type3).isAssignableFrom(rawType)) {
                    FieldType$$ExternalSyntheticBUOutline0.serializer("Type argument ", type2, " does not satisfy bounds for type variable ", typeVariable, " declared by ", type);
                    return null;
                }
            }
        }
    ...
    private Type getTypeTokenTypeArgument() {
        ...
        if (genericSuperclass instanceof ParameterizedType) {
            ...
            r8lambdayUqGVEpC1kg8ua3jc9a113shNIE.write(TroubleshootingGuide.createUrl("type-token-raw"), "TypeToken must be created with a type argument: new TypeToken<...>() {}; When using code shrinkers (ProGuard, R8, ...) make sure that generic signatures are preserved.\nSee ");
            return null;
        }
        ScreenStartObserver$$ExternalSyntheticLambda0.write("Must only create direct subclasses of TypeToken");
        return null;
    }
    ...
    private static void verifyNoTypeVariable(Type type) {
        ...
        if (!(type instanceof WildcardType)) {
            ...
            ScreenStartObserver$$ExternalSyntheticLambda0.IconCompatParcelizer("TypeToken captured `null` as type argument; probably a compiler / runtime bug");
            return;
        }
