...
public class FormatUtils {
    ...
    public static String createErrorMessage(String str, int i) {
        ...
        if (i <= 0) {
            return m1$$ExternalSyntheticOutline0.m('\"', "Invalid format: \"", strConcat);
        }
        if (i >= str.length()) {
            return ff$$ExternalSyntheticOutline0.m("Invalid format: \"", strConcat, "\" is too short");
        }
        StringBuilder sbM = c8$$ExternalSyntheticOutline0.m("Invalid format: \"", strConcat, "\" is malformed at \"");
        sbM.append(strConcat.substring(i));
        sbM.append('\"');
        return sbM.toString();
    }
    ...
    public static void writePaddedInteger(Writer writer, long j, int i) throws IOException {
        ...
        int iLog = (int) (Math.log(j) / LOG_10);
        while (i > iLog + 1) {
            writer.write(48);
            i--;
        }
